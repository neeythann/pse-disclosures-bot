...
`create_webhook(url, data)` (`src/main.py`) infers the sender class from the URL host via `urlparse`. `discord.com` / `discordapp.com` → `DiscordWebhook`; `hooks.slack.com` → `SlackWebhook`; any other host → `GenericWebhook` (a raw JSON listener). To add a provider, subclass `Webhook` and add a host branch in the factory. `_notify` calls `create_webhook` rather than constructing a sender directly. `SlackWebhook._format_data()` reuses the dataclass `to_embed()` and translates the Discord embed shape into a Slack `attachments` payload (`author.name`→`author_name`, `author.url`→`author_link`, `title`/`url`→`title`+`title_link`, `fields[].name`→`fields[].title`, `inline`→`short`, `footer.text`→`footer`); `to_embed()` stays the single source of truth and no dataclass changes are needed. `GenericWebhook._format_data()` visits `openDiscViewer.do?edge_no=` for the item, extracts the main-doc and attachment (`#file_list`) download links via `_fetch_disclosure_links` (cached per `edge_no`), and POSTs a raw payload: `{type, object (dataclasses.asdict), viewer_url, main_doc_url, attachment_links}` — no embed translation.
